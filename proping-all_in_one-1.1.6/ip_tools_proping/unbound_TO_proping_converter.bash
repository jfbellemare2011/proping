...
#			              ░         ░ ░            ░           ░       ░ 
#	
#-------------------------------------------unbound_file_TO_proping_converter------------------------------------------------------
#
...
FILE_IN=(`echo "$1"`)
        ...
        if [ -z "$FILE_IN" ]; then
                echo "Usage: [ give me a ( unbound.conf ) file !]"
                exit 1
        fi
...
# unbound_file to ./proping.conf

VAR=(`grep -E 'IN.*A' $FILE_IN | grep -v 'CNAME\|AAAA\|SOA\|#' | awk -F'"' '{print $1000,$2}' | awk '{$1000="*_";$1001="="; print $1000,$1,$1001,$4 }' >> ./proping.conf `)
VAR=(`grep -E ' forward-addr:' $FILE_IN | grep -v 'CNAME\|AAAA\|SOA\|#  ' | awk '{$1000="*_";$1001="="; print $1000,$1,$1001,$2 }' >> ./proping.conf `)
...
echo "The address DNS of unbound.conf file ( $FILE_IN ) has been converted to (./proping.conf)!"
