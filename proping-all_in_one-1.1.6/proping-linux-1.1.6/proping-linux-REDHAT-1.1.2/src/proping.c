...
/*****concatenate command for system(ping)****/
	...
	#define PING_COMMAND_STRCAT1 "ping "
	#define PING_COMMAND_STRCAT2 " -c 3 > /dev/null"
...
printf("\n		Pinging of interfaces; %s %s at %s",option_name, option_argument, ctime(&now));
		...
		sleep(3);
				...
				//-----------
					...
					strcat(wall_commandlog_strcat, "wall " );
...
//printf("String %d : %s\n",z+1,(*ptr_arr_downs)[z]);
	...
	strcat(wall_command_strcat, "wall !!!Warning!!! communication between " ); //---Concatenate for wall command
...
//	  char *trace_ip4;    //It have to declarate it as extern global variable
	...
	//--------------------------------------------------------------------------------------------------------
  	if( i != g ){ // This just to don't analysing the first line of tmp_trace.txt
