...
#The sendmail  mode, "on" "off", allows to start or stop sending warning mail of telecommunication network problems tcp/ip, by the processus ( /usr/lib/sendmail ) or by ( command telnet ).
...
D)sendmail_mode = off
E)sendmail_to = jf@localhost;user1@localhost;user2@localhost
...
#Configuration mode to send email by; | telnet | or | sendmail |
#If the choice is | telnet |, you have to specify mta_server_host and port of communication.
#Note; | telnet | can send e-mail to one user at a time, the first of the list; user1@exemple.com only

I)telnet_mta_server_host = localhost
J)telnet_mta_server_port = 25
K)sendingmail_by_telnet_or_sendmail = telnet
