...
printf("Please wait for a maximum of 3min, the time to finish traceroute analysis!!!\n\n\n");
...
strcpy(trace_command_strcat, "echo '(UNKNOW_HOST-OWN_INTERFACE_DOWN)' > /tmp/tmptrace.txt"); // File tmptrace is not emptys by the way
system(trace_command_strcat);

trace_ip4 = "UNKNOW_HOST-OWN_INTERFACE_DOWN"; //Trace_ip4 not = at NULL by this way
strcpy(trace_command_strcat, " ");

strcat(trace_command_strcat, "traceroute -m 15 ");//---Concatenate for traceroute command
strcat(trace_command_strcat, option_argument); 
strcat(trace_command_strcat, " > /tmp/tmptrace.txt");
system(trace_command_strcat);
...
//--------------------------------------------------------------------------------------------------------
if( i != g ){ // This just to not analysing the first line of tmp_trace.txt
